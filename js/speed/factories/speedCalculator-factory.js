(function () {
  ...
  function SpeedCalculatorFactory($q, BluetoothFactory, sensorMAC, LogsFactory) {
    ...
    function hasBeenDetected(data, position){
      if (isInEvent === false) {
        if (data[position] < (sensorBaseData[position] - TOLERANCE) || data[position] > (sensorBaseData[position] + TOLERANCE)) {
          isInEvent = true;
        }
      } else {
        if (data[position] >= (sensorBaseData[position] - TOLERANCE) && data[position] <= (sensorBaseData[position] + TOLERANCE)) {
    ...
    function startMeasuring(){
      ...
      BluetoothFactory.subscribe(function (noty) {
        ...
        var eventDetected;
        if (backwards === false) {
          eventDetected = hasBeenDetected(data, i);
        } else {
          var x = sensorBaseData.length - 1 - i;
          eventDetected = hasBeenDetected(data, x);
        }
        ...
        if (measures.length === 0 && eventDetected === false) {
          ...
          eventDetected = hasBeenDetected(data, y);
          if (eventDetected) {
            backwards = true;
          }
        }
